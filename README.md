...
- **?? Dual-Core FreeRTOS Architecture**: VW CDC protocol timings are incredibly strict (~1200?s / 3200?s). We use FreeRTOS "Core Pinning" to lock the CDC SPI interface to Core 1, while the WebUI and Wi-Fi stack run safely on Core 0. Absolute zero timing jitter.
...
Built upon the knowledge gathered by the open-source car-hacking community. Special thanks to original AVR concepts from shyd, 	omaskovacik, and recent architectural inspirations from NullString1.
